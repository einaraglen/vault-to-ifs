...
export const create_inventory_part = async (client: Connection, row: ExportPart) => {
  ...
  try {
    ...
    throw new IFSError((err as Error).message, "Create Inventory Part", row);
  }

  if (!res.ok) {
    throw new IFSError(res.errorText, "Create Inventory Part", row);
  }
