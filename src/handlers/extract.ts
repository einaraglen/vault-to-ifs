...
import { build_structure_chain, convert_to_struct, filter_unique_parts } from "../utils"
...
export const extract_transaction = async (mssql: ConnectionPool, transaction: string) => {
    ...
    const struct_chain = build_structure_chain(children, map)

    return { root: convert_to_struct(root), unique_parts: list, struct_chain }
}