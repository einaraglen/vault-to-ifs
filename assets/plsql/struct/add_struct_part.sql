PROCEDURE Add_Struct_Part__ IS
    info_               VARCHAR2(2000);
  attr_               VARCHAR2(2000);
...
BEGIN
  OPEN check_sub_struct(Get_Part_No__(:c02), :c03, Get_Part_No__(:c06), :c07);
  ...
  IF exists_ = 0 THEN
    ...
    &AO.Client_SYS.Add_To_Attr('PART_NO', Get_Part_No__(:c02), attr_);
    &AO.Client_SYS.Add_To_Attr('PART_REV', :c03, attr_);
    &AO.Client_SYS.Add_To_Attr('SUB_PART_NO', Get_Part_No__(:c06), attr_);
  ...
  IF :c09 LIKE '1' THEN
    ...
    &AO.Client_SYS.Add_To_Attr('PART_NO', Get_Part_No__(:c02), attr_);
    &AO.Client_SYS.Add_To_Attr('PART_REV', :c03, attr_);
    &AO.Client_SYS.Add_To_Attr('SPARE_PART_NO', Get_Part_No__(:c06), attr_);
  ...
  -- CHANGE LOGS
  OPEN get_struct_object(Get_Part_No__(:c02), :c03, Get_Part_No__(:c06), :c07);
    FETCH get_struct_object
      INTO objid_, objversion_;
  CLOSE get_struct_object;

  OPEN get_prev_rev(Get_Part_No__(:c02), :c03); 
    ...
    FOR a_ IN get_prev_added(Get_Part_No__(:c02), prev_part_rev_, Get_Part_No__(:c06), :c07 ) LOOP
      found_ := TRUE;
    END LOOP;
    ...
    ELSE
      ...
      FOR a_ IN get_prev_rev_chg(Get_Part_No__(:c02), prev_part_rev_, Get_Part_No__(:c06), :c07 ) LOOP
        found_ := TRUE;
        attr_ := NULL;
      ...
      IF NOT ( found_ ) THEN
        ...
        FOR a_ IN get_prev_qty(Get_Part_No__(:c02), prev_part_rev_, Get_Part_No__(:c06), :c07, :n01) LOOP
          found_ := TRUE;
          attr_ := NULL;
